...
export default {
  ...
  data() {
    return {
      ...
      ps: null,
      isOn: [false, false, false, false, false, false],
    };
  },
  mounted() {
    if (window.kakao && window.kakao.maps) {
      ...
      /* global kakao */
      script.onload = () => kakao.maps.load(this.initMap); // onload 시 카카오맵 로드
      ...
      document.head.appendChild(script); // head에 script 코드 추가
    }
  ...
  methods: {
    ...
    initMap() {
      ...
      var options = {
        //지도를 생성할 때 필요한 기본 옵션
        center: new kakao.maps.LatLng(37.50133446272347, 127.03967579316388), //지도의 중심좌표.
    ...
    onClickCategory(id, number) {
      ...
      if (this.isOn[number]) {
        this.currCategory = "";
        this.changeCategoryClass(-1);
        this.removeMarker();
      } else {
        this.currCategory = id;
        this.changeCategoryClass(number);
        this.searchPlaces();
      }
    },

    // 클릭된 카테고리에만 클릭된 스타일을 적용하는 함수입니다
    changeCategoryClass(number) {
      for (let i = 0; i < this.isOn.length; i++) {
        this.isOn[i] = false;
      }

      if (number >= 0) {
        this.isOn[number] = true;
      }
    },
  },
};
